...
//type is a reserved word in rust and other languages so we will use xdg_type
    ...
    /// Icon to display in file manager, menus, etc. If the name is an absolute path, the given file will be used. If the name is not an absolute path, the algorithm described in the (Icon Theme Specification)[http://freedesktop.org/wiki/Standards/icon-theme-spec] will be used to locate the icon.     iconStringing    NO    1-3
...
/// The same desktop name may not appear in both OnlyShowIn and NotShowIn of a group. Stringing(s)    NO    1-3
    ...
    /// Identifiers for application actions. This can be used to tell the application to make a specific action, different from the default behavior. (The Application actions section)[https://specifications.freedesktop.org/desktop-entry-spec/latest/ar01s11.html] describes how actions work.     Stringing(s)    NO    1
    pub actions:Option<Vec<String>>,
    /// The MIME type(s) supported by this application.     Stringing(s)    NO    1
    pub mime_type:Option<Vec<String>>,
    /// Categories in which the entry should be shown in a menu for possible values (see the Desktop Menu Specification)[http://www.freedesktop.org/Standards/menu-spec].     Stringing(s)    NO    1
    pub categories:Option<Vec<Categories>>,
    /// A list of interfaces that this application implements. By default, a desktop file implements no interfaces. (See Interfaces)[https://specifications.freedesktop.org/desktop-entry-spec/latest/ar01s09.html] for more information on how this works.     Stringing(s)    NO
    ...
    /// If true, it is KNOWN that the application will send a "remove" message when started with the DESKTOP_STARTUP_ID environment variable set. If false, it is KNOWN that the application does not work with startup notification at all (does not shown any window, breaks even when using StartupWMClass, etc.). If absent, a reasonable handling is up to implementations (assuming false, using StartupWMClass, etc.). (See the Startup Notification Protocol Specification for more details)[http://www.freedesktop.org/Standards/startup-notification-spec].     boolean    NO    1
    pub startup_notify:Option<bool>,
    /// If specified, it is known that the application will map at least one window with the given Stringing as its WM class or WM name hint (see the Startup Notification Protocol Specification for more details)[http://www.freedesktop.org/Standards/startup-notification-spec].     Stringing    NO    1
